...
Claude Codeを用いて作成した、arXiv APIを使って論文を検索・閲覧できるWebアプリ。
...
- **引用関係・類似論文は上位N件へ絞り、並び替え条件を選べる。** 1つの論文に対して引用/被引用が数十〜数百件になることがあり、そのまま全件出すと読みづらい。デフォルトは「多く引用されている論文ほど重要度が高い可能性が高い」という単純なヒューリスティックで`citationCount`（被引用数）順に上位を選ぶが、それ以外に「新しい順」「古い順」も選べるようにした（`app/api/citations/route.ts`の`selectTopN`、`app/api/related-papers-sort.ts`）。「古い順」はその分野で最初期の研究を見つけるのに、「新しい順」は最新の関連研究を見るのに使える。
  ...
  - **年間被引用数（citationsPerYear）を追加**: 「引用している論文（citations側）」を被引用数で絞ると単に古い論文の方が有利になりやすい、という上記の注意点への実際の対策として、`citationCount / 経過年数`で並べる選択肢を追加した（`app/api/related-papers-sort.ts`の`citationsPerYearScore`）。経過年数は最低1年として扱い、出たばかりの論文のスコアが分母の小ささで過剰に跳ね上がらないようにしている。公開日が不明な論文はスコア0（最下位）として扱う。
- **Semantic Scholarへのレスポンスはサーバー側でメモリキャッシュする。** 無認証APIはレート制限がかなり低く、同じ論文の引用/類似論文を何度も開閉するだけですぐ429（Too Many Requests）になっていたため、`app/api/semantic-scholar-cache.ts`にURL単位のキャッシュを用意し、citations・recommendations両方のRoute Handlerで共有している。TTLは24時間（citationデータは数日単位でしか変わらないため）。サーバープロセスが再起動すると消える程度の簡素な実装で今は十分。
  - **同時リクエストの一本化（in-flight coalescing）**: 同じURLへのリクエストが完了前に重なった場合（例: 関係マップを開くとcitations/recommendationsを同時に要求する）、2本目以降は新たに外部APIを叩かず、進行中のPromiseを共有する。
  - **上流への直列リクエスト**: Semantic Scholarの無認証プールは概ね秒1リクエスト程度のため、実際の外部リクエストは最短1.1秒間隔で直列に送るキュー（`arxiv-client.ts`と同じ考え方）を追加した。
  - **429時のstale-on-error**: 新規取得が429などで失敗した場合、期限切れでも古いキャッシュがあればそれを返し、エラーをそのままユーザーに見せないようにした（少し古い情報でもエラー画面よりまし、という判断）。この場合は5分間だけ再試行を控える。
...
### 2026-07-11: 時期の空白を圧縮、ホバーのツールチップ改善、クリックでのサイト内検索
- 課題1: 実際の日付をそのまま線形に縦軸へマッピングしていたため、論文が集中している時期は重なって見づらく、逆に論文が存在しない期間は無駄な空白になっていた。
- 対応1: 縦位置の計算を「実際の日付の連続値」から「論文が実在する日付だけを集めて重複排除し、順位（何番目に古いか）で均等配置する」方式に変更した。これにより存在しない期間はスペースを消費せず、密集していた時期は逆に間隔が広がる。同じ日付の論文はレーンをまたいで同じ高さに揃うため、時期の近さも読み取れる。
- 課題2: ノードにマウスを乗せた際、ブラウザ標準の`<title>`ツールチップ（表示が遅く見た目も調整できない）に頼っていた。
- 対応2: `<title>`をやめ、カーソル位置に追従する独自のツールチップ（タイトル・発表日・被引用数を表示するカード）を実装した。
- 課題3: マップ上のノードをクリックすると常にarXivの外部ページに飛ぶだけで、このサイトの中でその論文を深掘りする手段がなかった。
- 対応3: 円をクリックするとこのサイト自身の検索にその論文タイトルを投げて結果を表示する（`page.tsx`の`searchForPaper`）ようにし、arXivへの外部リンクは論文名のテキスト部分をクリックした場合に限定した。これによりマップ上から新しい論文を起点に探索を続けられる。

### 2026-07-11: 429対策の強化とレート制限時の専用UI
- 課題: キャッシュ・APIキー対応・1回のリトライを実装済みだったが、それでも頻繁にSemantic Scholarの429エラーに遭遇するようになった。エラーが起きたときの画面表示も素っ気ない赤文字だけだった。
- 追加のレート制限対策は`claude-fable-5`モデルにブレインストーミングを依頼し、費用対効果の高そうな案を採用した（`app/api/semantic-scholar-cache.ts`）。
  - **in-flightリクエストの一本化**: 同じURLへの同時リクエストは1本のPromiseを共有し、外部APIへの二重発火を防ぐ。
  - **上流への直列リクエスト（最短1.1秒間隔）**: arXiv用のクライアントと同じ考え方で、Semantic Scholarへの実リクエストにも直列キューを導入。
  - **キャッシュTTLを30分→24時間に延長**: citation/referenceデータは数日単位でしか変わらないため。
  - **429時のstale-on-error**: 新規取得が失敗しても、期限切れの古いキャッシュがあればそれを返してエラーを隠す（5分間は再試行を控える）。
  - **Next.jsのData Cache（`next: { revalidate }`）を併用**: Vercelのサーバーレス環境では自前のメモリキャッシュがインスタンスをまたいで保持されない可能性があるため、Next.js自体のキャッシュ機構でも補完する。
  - 見送った案: Redis等の外部キャッシュストア（個人プロジェクトの規模には過剰）、レート制限用レスポンスヘッダーの解析（Semantic Scholarが安定して返してくれるとは限らない）。
- UI側の対応: `PaperCard.tsx`のフックにHTTPステータスコードと直前のリクエストパラメータを保持させ、429のときだけ通常のエラー文言ではなく、状況の説明・APIキー設定への案内・「再試行する」ボタン付きの専用バナー（`RateLimitNotice`）を表示するようにした。

### 2026-07-11: 年間被引用数ソートの追加、関係マップのはみ出しバグ修正
- ユーザーからの技術的な改善提案5件をレビューし、費用対効果の高い2件を採用した（残りは既に対応済み・見送りと判断し理由を回答済み）。
- **年間被引用数（citationsPerYear）ソートを追加**: `citationCount`のみでソートすると古い論文が有利になりやすいという、以前から自覚していたバイアスへの対策。`citationCount / 経過年数`（経過年数は最低1年）で並べる選択肢を`app/api/related-papers-sort.ts`の`citationsPerYearScore`として実装し、citations・recommendations両方のRoute HandlerとUIのセレクトボックスに追加した。公開日が不明な論文はスコア0として最下位に回る。
- **関係マップのSVGはみ出しバグを修正**: 同じ日付（同じランク）に多数の論文が集中すると、`deoverlapY`による押し出しが当初の高さの見積もり（`maxLaneCount`ベース）を超え、ノードがキャンバス外に見切れる可能性があった。`deoverlapY`適用後の実際のノードのY座標の最大値を走査し、それに応じて`viewBox`の高さを動的に決定するよう修正した（`app/components/RelationMap.tsx`）。
- **モバイルでの視認性対応**: SVGが画面幅に合わせて縮小され文字が読めなくなる問題に対し、SVGを実サイズ（`width`/`height`固定）で描画し、親要素に`overflow-x-auto`を指定して横スクロールで閲覧できるようにした。

### 2026-07-11: 関係マップの配置ロジックを刷新（基準線越え防止・グループ化によるコンパクト化）
- 課題1: 前回のはみ出し修正は「高さの見積もり」の問題は直したが、根本の配置アルゴリズム（`deoverlapY`が単純に下方向へ押し出すだけ）自体は変えていなかった。そのため先行研究が密集すると基準線（選んだ論文の発表時期）を越えて後続研究側に食い込んだり、データソースの日付の食い違いにより後続研究が基準線より上に表示されたりすることがあった。
- 課題2: 表示件数を増やすと、1レーン＝1列に積み上げる設計のため縦にどんどん伸びてしまい、見た目も操作性も悪化していた。
- 対応: 配置ロジックを作り直した（`app/components/RelationMap.tsx`）。
  - 近い時期の論文は`clusterByProximity`でグループにまとめ、`layoutClusters`でグループ内を横に並べる（最大4列、超えたら下の行へ）。縦一列ではなくコンパクトな塊として表示されるため、件数が増えても縦の伸びを抑えられる。
